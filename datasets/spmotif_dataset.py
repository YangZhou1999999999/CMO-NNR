...
from torch_geometric.utils import remove_self_loops, add_self_loops, to_undirected, degree
import copy
...
class SPMotif(InMemoryDataset):
    ...
    def process(self, get_data_list=False):
        ...
        if 'tspmotif' in self.root.lower() or 'dspmotif' in self.root.lower():
            edge_index_list, label_list, base_list, ground_truth_list, role_id_list, pos, motifs = np.load(osp.join(self.raw_dir, self.raw_file_names[idx]), allow_pickle=True)
        elif '0.6' in self.root or '0.7' in self.root:
            edge_index_list, label_list, base_list, ground_truth_list, pos, = np.load(osp.join(self.raw_dir, self.raw_file_names[idx]), allow_pickle=True)
        ...
        data_list = []
        for idx, (edge_index, y, group, ground_truth, z, p, mid) in enumerate(zip(edge_index_list, label_list, base_list, ground_truth_list, role_id_list, pos, motifs)):
            edge_index = torch.from_numpy(edge_index)
            edge_index = edge_index.long()
            node_idx = torch.unique(edge_index)
            assert node_idx.max() == node_idx.size(0) - 1
            x = torch.zeros(node_idx.size(0), 7)
            ...
            try:
                if len(group) > 0:   # group: environment id
                    group = -1
            ...
            #     edge_index = to_undirected(edge_index, num_nodes=n_node)
            ...
            #     return edge_index, edge_weight_gcn
            
            # edge_index, train_edge_weight_gcn = normalize_edge(edge_index, x.shape[0])
            # edge_attr = torch.ones(edge_index.size(1), 1)
            data = Data(x=x,
                        y=y,
                        ...
                        # train_edge_weight_gcn=train_edge_weight_gcn,
                        # edge_gt_att=torch.LongTensor(ground_truth), # edge label
                        idx=torch.LongTensor([idx]),   
                        mid=torch.LongTensor([mid]),    # motif id
                        group=torch.LongTensor([group]))    # base id
            # add self loops
