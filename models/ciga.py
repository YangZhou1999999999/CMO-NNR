...
from utils.util import node_id_list_to_mask, to_np, to_tensor, projection_simplex
...
class GNNERM(nn.Module):
    ...
    def get_groupdro_loss(self, batched_data, criterion, is_labeled, return_data="pred", return_spu=True):
        causal_pred, causal_rep = self.forward(batched_data, return_data="rep")
        return causal_pred, causal_rep, torch.zeros(())
...
class CIGA(nn.Module):
    ...
    def __init__(self,
        ...
        ## GNN to generate node embeddings：Rationale Generator
        if gnn_type.lower() == "le":
            self.gnn_encoder = LeGNN(in_channels=input_dim,
                                     hid_channels=emb_dim,
                                     num_layer=num_layers,
        ...
        # Shared Graph Encoder + Classifier
        self.classifier = GNN(gnn_type=gnn_type,
        ...
        # Learnable q
        # V1
        self.q0 = torch.ones(self.src_domain)
        self.q0 /= self.q0.sum()
        self.register_buffer("q", self.q0)
        # ablation study initialize q like this
        # V2
        # self.register_buffer("q", torch.Tensor())
        self.groupdro_eta = 0.5
        self.lmbda = 1 # [0.5, 0.8, 1, 1.2], can control the performance stability
    ...
    def split_graph(self,data, edge_score, ratio):
        ...
        return (new_causal_edge_index, new_causal_edge_attr, new_causal_edge_weight), (new_spu_edge_index, new_spu_edge_attr, new_spu_edge_weight)
    ...
    def forward(self, batch, pred_edge_weight=None, return_data="pred", return_spu=False, debug=False):
        ...
        if self.ratio < 0:
            (causal_edge_index, causal_edge_attr, causal_edge_weight), (spu_edge_index, spu_edge_attr, spu_edge_weight) = (batch.edge_index, batch.edge_attr, pred_edge_weight), (batch.edge_index, batch.edge_attr, pred_edge_weight)
        else:
            (causal_edge_index, causal_edge_attr, causal_edge_weight), (spu_edge_index, spu_edge_attr, spu_edge_weight) = self.split_graph(batch, pred_edge_weight, self.ratio)

        # # valid topology-aware
        # # casual nodes for computing embedding distance
        # num_nodes = torch.unique(batch.edge_index).size(0)  # batch.num_nodes
        # casual_node_id = torch.unique(causal_edge_index).tolist() # casual_node_id.size(0)
        # casual_node_mask = node_id_list_to_mask(casual_node_id, num_nodes)
        # casual_reps = h[casual_node_mask]
        # sup_reps = h[np.logical_not(np.array(casual_node_mask))]
        ...
        # elif return_data.lower() == "dist":
        #     return causal_pred, casual_node_id, h
        elif return_data.lower() == "dro":
            return causal_pred, causal_rep, pred_edge_weight
        else:
            return (causal_x, causal_edge_index, causal_edge_attr, causal_edge_weight, causal_batch), (spu_x, spu_edge_index, spu_edge_attr, spu_edge_weight, spu_batch), pred_edge_weight
    ...
    def get_disc_loss(self, batched_data, labels, criterion, is_labeled=None, return_data="pred", grad=False):
        (causal_x, causal_edge_index, causal_edge_attr, causal_edge_weight, causal_batch), \
             (spu_x, spu_edge_index, spu_edge_attr, spu_edge_weight, spu_batch), pred_edge_weight =  \
                self.forward(batched_data, return_data="")
        if is_labeled == None:
            is_labeled = torch.ones(labels.size()).to(labels.device)
    ...
    # our final version
    def get_groupdro_loss(self, batched_data, criterion, is_labeled, return_data="pred", return_spu=True):
        causal_pred, causal_rep, pred_edge_weight = self.forward(batched_data, return_data="dro", return_spu=False)
        ...
        for m in range(self.src_domain):
            ...
            if torch.sum(mask).item() < 2:
                return causal_pred, causal_rep, torch.zeros(())
            # compute consistency
            distances = torch.norm(causal_rep[mask], dim=1)
        ...
        loss_consistency_pred = torch.dot(losses, self.q.to(torch.float))
        return causal_pred, causal_rep, loss_consistency_pred
    
    # ablation study
    def get_groupdro_lossV2(self, batched_data, criterion, is_labeled, return_data="pred", return_spu=True):
        causal_pred, causal_rep, pred_edge_weight = self.forward(batched_data, return_data="dro", return_spu=False)
        labels = batched_data.y
        ...
        for m in range(self.src_domain):
            ...
            # var represents consistency
            variance = torch.mean(torch.var(distances, dim=0)).to('cpu')
            losses[m] = var_eta * variance
            # losses[m] = criterion(causal_pred[mask], labels[mask]) 
            self.q[m] *= (self.groupdro_eta * losses[m].data).exp()

        self.q /= self.q.sum()
        
        loss_consistency_pred = torch.dot(losses, self.q)
        return causal_pred, causal_rep, loss_consistency_pred
...
class GALA(nn.Module):
    ...
    def forward(self, batch, return_data="pred", return_spu=False, debug=False):
        ...
        if return_data.lower() == "pred":
            ...
            return causal_pred, causal_rep, h
        elif return_data.lower() == "feat": # featurizer's representation
            causal_h, _, __, ___ = relabel(h, causal_edge_index, batch.batch)
            if self.c_pool.lower() == "add":
                casual_rep_from_feat = global_add_pool(causal_h, batch=causal_batch)
            elif self.c_pool.lower() == "max":
