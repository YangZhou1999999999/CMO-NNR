    ...
    def __init__(self,
        ...
        if self.graph_pooling == "sum":
            ...
            self.pool = GlobalAttention(gate_nn=torch.nn.Sequential(torch.nn.Linear(emb_dim, 2 * emb_dim), 
                                                                    torch.nn.BatchNorm1d(2 * emb_dim), torch.nn.ReLU(), torch.nn.Linear(2 * emb_dim, 1)))
        ...
        if pred_head == "cls":
            ...
            self.graph_pred_linear = torch.nn.Sequential(nn.Linear(emb_dim, 2 * emb_dim), nn.ReLU(), nn.Linear(2 * emb_dim, self.num_class))
            self.spu_mlp = torch.nn.Sequential(nn.Linear(emb_dim, 2 * emb_dim), nn.ReLU(), nn.Linear(2 * emb_dim, self.num_class))
            # self.graph_pred_linear = torch.nn.Linear(self.emb_dim, self.num_class)
