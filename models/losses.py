...
def get_irm_loss(causal_pred, labels, batch_env_idx, criterion=F.cross_entropy):
    ...
    # fixed "dummy" classifier: 通常是一个非常简单的模型，例如一个恒等映射（identity mapping）
    ...
    # 在 IRM 方法中，使用 "dummy" classifier 的目的是为了检验数据表示是否具有不变性。
    # 通过检查 "dummy" classifier 在不同环境下的表现，我们可以了解数据表示是否跨环境一致。
...
def get_contrast_loss(causal_rep, labels, norm=None, contrast_t=1.0, sampling='mul', y_pred=None, env_label=False):
    ...
    if sampling.lower() in ['mul', 'var']:
        ...
        # motif anchor       
        anchor_motif_mean_log_prob_pos = 0
        for m in torch.unique(labels):
            mask = (labels == m)
            selected_samples = causal_rep[mask]
            center = torch.mean(selected_samples, dim=0)
            positive_similarities = torch.matmul(selected_samples, center) / contrast_t
            all_similarities = torch.matmul(causal_rep, center) / contrast_t
            negative_similarities = all_similarities[~mask]
            positive_logits = torch.exp(positive_similarities)
            negative_logits = torch.sum(torch.exp(negative_similarities))
            anchor_motif_mean_log_prob_pos = -torch.log(positive_logits / (positive_logits + negative_logits)).mean()
        # loss
        contrast_loss = -mean_log_prob_pos.mean() + anchor_motif_mean_log_prob_pos
        ...
        # hard positive: diff pred label && same ground truth label
